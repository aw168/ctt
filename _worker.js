...
async function checkAndRepairTables(d1) {
  ...
  for (const [tableName, structure] of Object.entries(expectedTables)) {
    try {
      const tableInfo = await d1.prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name=?`).bind(tableName).first();
      if (!tableInfo) {
      ...
      const currentColumns = new Map(columnsResult.results.map(col => [col.name, col]));
      for (const [colName, colDef] of Object.entries(structure.columns)) {
        if (!currentColumns.has(colName)) {
          ...
          const addColumnSQL = `ALTER TABLE ${tableName} ADD COLUMN ${colName} ${colDef.split(' ').slice(1).join(' ')}`;
...
async function createTable(d1, tableName, structure) {
  const columnsDef = Object.entries(structure.columns).map(([name, def]) => `${name} ${def}`).join(', ');
...
async function cleanExpiredVerificationCodes(d1) {
  ...
  try {
    ...
    const expiredCodes = await d1.prepare('SELECT chat_id FROM user_states WHERE code_expiry IS NOT NULL AND code_expiry < ?')
      .bind(nowSeconds).all();
    ...
    if (expiredCodes.results.length > 0) {
      ...
      for (const { chat_id } of expiredCodes.results) {
        await d1.prepare('UPDATE user_states SET verification_code = NULL, code_expiry = NULL WHERE chat_id = ?')
          .bind(chat_id).run();
...
async function onMessage(message) {
  ...
  let userState;
  try {
    userState = await env.D1.prepare('SELECT is_blocked, is_first_verification, is_deleted FROM user_states WHERE chat_id = ?')
      .bind(chatId).first();
  ...
  const isDeleted = userState?.is_deleted || false;
  ...
  if (isDeleted && text !== '/start') {
    console.log(`User ${chatId} is deleted, prompting reactivation`);
    await sendMessageToUser(chatId, '您的账户已被删除，请使用 /start 重新激活。');
    return;
  }
  ...
  if (text === '/start') {
    ...
    if (!userState) {
      ...
      try {
        await env.D1.prepare('INSERT INTO user_states (chat_id, is_first_verification) VALUES (?, ?)')
          .bind(chatId, true).run();
        userState = { is_first_verification: true, is_blocked: false, is_deleted: false };
    ...
    if (isDeleted) {
      console.log(`Reactivating deleted user ${chatId}`);
      await env.D1.prepare('UPDATE user_states SET is_deleted = ? WHERE chat_id = ?')
        .bind(false, chatId).run();
    }
...
async function sendAdminPanel(chatId, topicId, privateChatId, messageId) {
  ...
  try {
    const settings = await Promise.all([
      getSetting('verification_enabled'),
      getSetting('user_raw_enabled')
    ]);
    verificationEnabled = settings[0] === 'true';
    userRawEnabled = settings[1] === 'true';
    console.log('Fetched settings for admin panel:', { verificationEnabled, userRawEnabled });
  } catch (error) {
  ...
  try {
    ...
    console.log('Admin panel sent and previous message deleted');
  } catch (error) {
...
async function getVerificationSuccessMessage() {
  ...
  try {
    ...
    console.log(`user_raw_enabled: ${userRawEnabled}`);
  } catch (error) {
  ...
  try {
    ...
    console.log('Fetched verification success message:', trimmedMessage);
...
async function getNotificationContent() {
  try {
    ...
    console.log('Fetched notification content:', content.trim());
    return content.trim() || '';
  } catch (error) {
...
async function checkStartCommandRate(chatId) {
  ...
  await env.D1.prepare('INSERT OR REPLACE INTO message_rates (chat_id, start_count, start_window_start) VALUES (?, ?, ?)')
    .bind(chatId, data.count, data.start).run();
...
async function getSetting(key) {
  const result = await env.D1.prepare('SELECT value FROM settings WHERE key = ?').bind(key).first();
  ...
  if (data.startsWith('verify_')) {
    ...
    if (result === 'correct') {
      ...
      await env.D1.prepare('UPDATE user_states SET is_verified = ?, verified_expiry = ?, verification_code = NULL, code_expiry = NULL, last_verification_message_id = NULL WHERE chat_id = ?')
        .bind(true, verifiedExpiry, chatId).run();
    ...
    if (data.startsWith('block_')) {
      ...
      console.log(`Marking user ${privateChatId} as deleted`);
      try {
        await env.D1.prepare('UPDATE user_states SET is_deleted = ? WHERE chat_id = ?')
          .bind(true, privateChatId).run();
        await sendMessageToTopic(topicId, `用户 ${privateChatId} 已被标记为删除，但话题保留。`);
      } catch (error) {
        console.error(`Error marking user ${privateChatId} as deleted:`, error);
        await sendMessageToTopic(topicId, `标记用户 ${privateChatId} 删除失败：${error.message}`);
      }
    }
  ...
  if (lastVerificationMessageId) {
    try {
      await fetchWithRetry(`https://api.telegram.org/bot${BOT_TOKEN}/deleteMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chat_id: chatId, message_id: lastVerificationMessageId })
      });
    } catch (error) {
...
async function sendVerification(chatId) {
  ...
  await env.D1.prepare('INSERT OR REPLACE INTO user_states (chat_id, verification_code, code_expiry) VALUES (?, ?, ?)')
    .bind(chatId, correctResult.toString(), codeExpiry).run();
  ...
  try {
    ...
    await env.D1.prepare('UPDATE user_states SET last_verification_message_id = ? WHERE chat_id = ?')
      .bind(data.result.message_id.toString(), chatId).run();
  } catch (error) {
...
async function checkIfAdmin(userId) {
  ...
  try {
    const requestBody = JSON.stringify({ chat_id: GROUP_ID, user_id: userId });
