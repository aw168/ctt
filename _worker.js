...
// 添加全局开关
let verificationEnabled = true;  // 验证码默认开启
let rawDisplayEnabled = true;    // raw显示默认开启
  ...
  async fetch(request, env) {
    ...
    async function onMessage(message) {
        ...
        if (topicId) {
          ...
          if (privateChatId) {
            if (text.startsWith('/admin')) {
              await showAdminPanel(message, topicId, privateChatId);
              return;
            }
      ...
      if (text === '/start') {
        ...
        if (isFirstVerification && verificationEnabled) {
    ...
    async function showAdminPanel(message, topicId, privateChatId) {
      ...
      const inlineKeyboard = [
        [
          { text: isBlocked ? '解除拉黑' : '拉黑用户', callback_data: `admin_block_${privateChatId}` },
          { text: '检查黑名单', callback_data: `admin_check_${privateChatId}` },
        ],
        [
          { text: verificationEnabled ? '关闭验证码' : '开启验证码', callback_data: 'admin_toggle_verification' },
          { text: rawDisplayEnabled ? '关闭Raw显示' : '开启Raw显示', callback_data: 'admin_toggle_raw' },
        ],
        [
          { text: 'GitHub项目', url: 'https://github.com/iawooo/ctt/' },
          { text: '删除用户数据', callback_data: `admin_delete_${privateChatId}` },
        ]
      ];
    ...
    async function onCallbackQuery(callbackQuery) {
      ...
      if (data.startsWith('verify_')) {
        ...
        const [_, action, targetChatId] = data.split('_');
        const isAdmin = await checkIfAdmin(callbackQuery.from.id.toString());
        if (!isAdmin) {
        ...
        if (action === 'block') {
          ...
          const userState = await env.D1.prepare('SELECT is_blocked FROM user_states WHERE chat_id = ?')
            ...
            .first();
          const isBlocked = userState ? userState.is_blocked : false;
          await sendMessageToTopic(topicId, `用户 ${targetChatId} 是否在黑名单中：${isBlocked ? '是' : '否'}`);
          ...
          await env.D1.prepare('DELETE FROM chat_topic_mappings WHERE chat_id = ?')
            .bind(targetChatId)
            .run();
          await sendMessageToTopic(topicId, `用户 ${targetChatId} 的所有数据已删除，下次发消息将创建新话题。`);
        }
      } else if (data === 'admin_toggle_verification') {
        const isAdmin = await checkIfAdmin(callbackQuery.from.id.toString());
        if (isAdmin) {
          verificationEnabled = !verificationEnabled;
          await sendMessageToTopic(topicId, `验证码已${verificationEnabled ? '开启' : '关闭'}`);
        }
      } else if (data === 'admin_toggle_raw') {
        const isAdmin = await checkIfAdmin(callbackQuery.from.id.toString());
        if (isAdmin) {
          rawDisplayEnabled = !rawDisplayEnabled;
          await sendMessageToTopic(topicId, `Raw显示已${rawDisplayEnabled ? '开启' : '关闭'}`);
        }
      }
    ...
    async function copyMessageToTopic(topicId, message) {
      await fetchWithRetry(`https://api.telegram.org/bot${BOT_TOKEN}/copyMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          chat_id: GROUP_ID,
